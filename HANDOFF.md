...
1. **Secrets are distributed in the launcher scripts.** The launcher scripts (`start.command` / `start.bat`) embed real secrets and write them into `server/.env` so non-technical testers can run the app locally with zero setup. The embedded keys are the Supabase service-role key (bypasses all RLS — full admin access to the database), the Supabase anon key, and the Groq API key (`GROQ_API_KEY`). Anyone with a copy of the repo/scripts has all three. **As soon as local testing wraps up, rotate every embedded secret at its source** — the service-role and anon keys in Supabase (Project Settings → API) and the Groq key in the Groq console — and remove the embedded values from the scripts before any wider distribution.
